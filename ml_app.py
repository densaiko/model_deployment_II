...
def run_ml_app():
    st.title('Welcome to Employee Prediction')
    st.subheader("Machine Learning Section")

    with st.expander("Attribution Info"):
    ...
    simple_sample = np.array(encoded_result).reshape(1, -1)

    model = load_model("model_grad.pkl")

    prediction = model.predict(simple_sample)
    prediction_probability = model.predict_proba(simple_sample)
    ...
    if prediction == 1:
        ...
        st.warning("Please give more contributions")
        st.write(pred_probability_score)
